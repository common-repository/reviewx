...
?>
    ...
    public function reviewx_submit_from_myorder()
    {
        ...
        for ($i = 0; $i < count($forminput); $i++) {
            ...
            if ($forminput[$i]['name'] == "rx-default-star") :
                $default_rating = sanitize_text_field((int)$forminput[$i]['value']);
            endif;
        }
        ...
        if (\ReviewX_Helper::is_multi_criteria('product')) {
            ...
            $criteriaData = array_map(function($value) {
                if ($value > 5) {
                    return 5;
                } elseif ($value < 1) {
                    return 1;
            ...
            $total_rating_sum           = array_sum($criteriaData);
    ...
    public function rx_front_end_review_submit()
    {
        ...
        for ($i = 0; $i < count(wp_unslash($_POST['formInput'])); $i++) {
            error_log("aaa" . print_r(wp_unslash($_POST['formInput']), true));
        ...
        if (\ReviewX_Helper::is_multi_criteria($post_type)) {
            ...
            $total_rating_sum       = array_sum(array_values($criteriaData));
            $rating                 = round($total_rating_sum / count($criteriaData));
        } else {
